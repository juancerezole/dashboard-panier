...
function updateSourceInfo() {
  const files = state.data.sources.map(s => s.file).join(' · ');
  const ts = state.data.generatedAt ? new Date(state.data.generatedAt) : null;
  const tsTxt = ts && !isNaN(ts) ? ts.toLocaleString('es-AR', { dateStyle: 'short', timeStyle: 'short' }) : '';
  $('#sourceInfo').textContent = tsTxt
    ? `Última actualización: ${tsTxt} · Fuentes: ${files}`
    : `Fuentes: ${files}`;
}
...
function setupRefreshButton() {
  ...
  if (!btn) return;
  const ready = cfg.githubOwner && cfg.githubRepo && cfg.githubWorkflow && cfg.githubToken;
  if (!ready) { btn.hidden = true; return; }
  btn.hidden = false;
  btn.addEventListener('click', refreshViaGithub);
}

const sleep = ms => new Promise(r => setTimeout(r, ms));

async function refreshViaGithub() {
  const cfg = window.__CONFIG__ || {};
  const btn = $('#refreshBtn');
  ...
  const setLabel = (txt) => {
    btn.innerHTML = `<span class="refresh-icon spinning" aria-hidden="true">↻</span><span class="refresh-label">${txt}</span>`;
  };
  setLabel('Disparando…');
  $('#error').hidden = true;
  ...
  const base = `https://api.github.com/repos/${cfg.githubOwner}/${cfg.githubRepo}`;
  const triggeredAt = new Date(Date.now() - 5000); // margen de 5s para reloj

  try {
    // 1. Disparar workflow_dispatch
    const dispatch = await fetch(`${base}/actions/workflows/${cfg.githubWorkflow}/dispatches`, {
      method: 'POST',
      headers: { ...headers, 'Content-Type': 'application/json' },
      body: JSON.stringify({ ref: cfg.githubBranch || 'main' }),
    });
    if (!dispatch.ok) {
      const t = await dispatch.text();
      throw new Error(`No se pudo disparar el workflow (${dispatch.status}). ${t.slice(0, 200)}`);
    }

    // 2. Esperar a que aparezca la run y conseguir su id
    setLabel('Buscando ejecución…');
    let runId = null;
    for (let i = 0; i < 15 && !runId; i++) {
      await sleep(2000);
      const list = await fetch(`${base}/actions/runs?event=workflow_dispatch&per_page=10`, { headers });
      if (!list.ok) continue;
      const data = await list.json();
      const candidate = (data.workflow_runs || []).find(r =>
        new Date(r.created_at) >= triggeredAt &&
        (r.path?.endsWith(cfg.githubWorkflow) || r.name)
      );
      if (candidate) runId = candidate.id;
    }
    if (!runId) throw new Error('No apareció la ejecución en GitHub. Verificá permisos del token.');

    // 3. Polling hasta que la run termine
    setLabel('Actualizando datos…');
    const startPoll = Date.now();
    while (true) {
      if (Date.now() - startPoll > 5 * 60 * 1000) {
        throw new Error('Timeout esperando al workflow (>5 min).');
      }
      await sleep(3000);
      ...
      if (run.status === 'completed') {
        if (run.conclusion !== 'success') {
          throw new Error(`El workflow terminó con estado: ${run.conclusion}. Revisá logs en GitHub.`);
        }
        break;
      }
    }

    // 4. Recargar data.json desde el hosting con cache-buster
    setLabel('Cargando…');
    const r = await fetch(`data.json?_=${Date.now()}`, { cache: 'no-store' });
    if (!r.ok) throw new Error('No se pudo leer data.json del hosting.');
    const fresh = await r.json();
    if (!fresh || !fresh.years) throw new Error('data.json sin datos válidos.');

    state.data = fresh;
    window.__DATA__ = fresh;
    const years = Object.keys(fresh.years).sort();
    ...
    const err = $('#error');
    err.hidden = false;
    err.innerHTML = `<div><strong>No se pudo actualizar:</strong> ${e.message}<br/><br/>
      Revisá la configuración en <code style="background:#000;padding:.2rem .4rem;border-radius:6px;">config.js</code>
      y los logs del workflow en GitHub Actions.</div>`;
